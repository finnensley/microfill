...
- Development Setup
  ...
  5. Copy .env.example to .env.local and paste the generated NEXT_PUBLIC_SUPABASE_URL, NEXT_PUBLIC_SUPABASE_ANON_KEY, and SUPABASE_SERVICE_ROLE_KEY values.
     Set `DEFAULT_TENANT_ID` to suggest a default tenant during local onboarding. It no longer auto-assigns users silently.

  6. Reset and seed the local database from migrations: npm run supabase:reset

  6a. Regenerate typed database definitions after schema changes: npm run supabase:types 7. Run the app: npm run dev 8. Only use a tunnel for webhook testing against real Shopify or ShipHero callbacks. Day-to-day UI and database work should stay local.
